...
function renderAdminCard(m) {
  ...
  let pensBlock = '';
  if (pensOn) {
    const suddenDeath = ph === pa && ph >= 5 && pa >= 5;
    const phase = suddenDeath
      ? 'SUDDEN DEATH — one each until a winner'
      : 'First 5 each — if still level, sudden death';
    pensBlock = `
        <div class="mt-3 p-3 rounded-xl ${suddenDeath ? 'bg-red-500/15 border border-red-500/40' : 'bg-amber-500/10 border border-amber-500/30'}">
          ...
          <p class="text-[11px] text-slate-400 mt-2 leading-snug">
            ${suddenDeath
              ? 'Still level after 5 each. Keep adding +1 for each successful kick until one team leads.'
              : 'Tap +1 pen only when a kick is scored. After both have 5 successful pens and scores are still equal → Sudden Death mode.'}
          </p>
        </div>`;
  } else if (hs === as) {
  ...
  const goalsList = matchGoals.length
    ? `<div class="mt-3 text-sm space-y-1">
        <p class="text-xs text-slate-400 mb-1">Goals (tap X to remove)</p>
        ${matchGoals.map(g => `
          <div class="flex justify-between items-center bg-slate-700/60 rounded-lg px-3 py-2">
            <span>Goal ${g.player_name} <span class="text-slate-400">(${getTeamName(g.team_id)})</span> ${g.minute ? g.minute + "'" : ''}</span>
            <button onclick="deleteGoal('${g.id}', '${m.id}', '${g.team_id === m.home_team_id ? 'home' : 'away'}')"
              class="text-red-400 hover:text-red-300 font-bold text-lg leading-none px-2">X</button>
          </div>
        `).join('')}
       </div>`
    : '';
    ...
    ? `<div class="mt-3 text-sm space-y-1">
        ...
        ${matchCards.map(c => `
          <div class="flex justify-between items-center bg-slate-700/60 rounded-lg px-3 py-2">
            ...
            <button onclick="deleteCard('${c.id}')" class="text-red-400 hover:text-red-300 font-bold text-lg leading-none px-2">X</button>
          </div>
        `).join('')}
       </div>`
    : '';
  ...
  return `
    <div class="bg-slate-800 rounded-2xl p-5 border border-slate-700" data-id="${m.id}">
      ...
      <div class="mb-4 p-3 rounded-xl border border-slate-600 bg-slate-900/50">
        ...
        ${!pensOn ? `
        <button type="button" onclick="startPens('${m.id}')"
          class="w-full py-3 mb-2 rounded-xl bg-amber-500 text-slate-900 font-bold text-sm">Start penalties</button>
        ` : `
...
window.resetScore = async function(matchId) {
  ...
  await sb.from('matches').update({
    ...
    pens_on: false, pen_home: 0, pen_away: 0,
...
window.startPens = async function(matchId) {
  const { error } = await sb.from('matches').update({
    pens_on: true, pen_home: 0, pen_away: 0,
    status: 'penalties',
    updated_at: new Date().toISOString()
  }).eq('id', matchId);
  if (error) alert('Error: ' + error.message);
