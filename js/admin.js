...
function renderAdminCard(m) {
            ...
            <div class="flex justify-between items-center bg-slate-900/80 rounded-lg px-3 py-2 text-sm">
              ...
              <button
                ...
                onclick="deleteGoal(
                  '${g.id}',
                  '${m.id}',
                  '${g.team_id === m.home_team_id ? 'home' : 'away'}'
                )"
  ...
  if (pensOn) {
    const normalLimitReached =
      !suddenDeath &&
      ph >= 5 &&
      pa >= 5;

    const homeNormalLimitReached =
      !suddenDeath &&
      ph >= 5;

    const awayNormalLimitReached =
      !suddenDeath &&
      pa >= 5;
    ...
    pensUI = `
      <div class="mt-4 p-4 rounded-2xl ${
        ...
        ${
          normalLimitReached
            ? `
              <div class="text-center text-xs font-bold text-red-400 mb-3">
                Five penalties each completed — sudden death required.
              </div>
            `
            : ''
        }
  ...
  return `
    <div class="bg-slate-800 rounded-2xl border border-slate-700 p-4 space-y-4">
      ...
      <div class="grid grid-cols-3 gap-2">
        ...
        <button
          type="button"
          onclick="${pensOn ? 'void(0)' : `startPens('${m.id}')`}"
...
window.enterSuddenDeath =
  async function (matchId) {
    ...
    if (!m.pens_on &&
        m.status !== 'penalties') {
      alert(
        'Penalty shootout has not started.'
      );
      return;
    }
    ...
    if (
      ...
      alert(
        'Sudden death can only begin after both teams have taken five penalties and the score is tied at 5–5.'
      );
      return;
    }
    ...
    if (m.pens_sudden) {
      alert(
        'This shootout is already in sudden death.'
      );
      return;
    }
...
window.changePen =
  async function (
    ...
    if (
      ...
      alert(
        'This penalty shootout is already finished.'
      );
      return;
    }
    ...
    /*
     * HARD LIMIT:
     * In normal penalty phase each side
     * may score at most five penalties.
     */
    if (
      delta > 0 &&
      !suddenDeath
      ...
      if (
        ...
        alert(
          'Home team has already taken all five normal penalties.'
        );
        return;
      }
      ...
      if (
        ...
        alert(
          'Away team has already taken all five normal penalties.'
        );
        return;
      }
    }
    ...
    /*
     * Normal phase:
     * once both teams reach 5–5, automatically
     * switch the shootout into sudden death.
     */
    let nextSuddenDeath =
      suddenDeath;
    ...
    /*
     * Sudden death:
     * the first score that creates a lead
     * finishes the shootout.
     */
    let nextStatus =
      'penalties';
    ...
    /*
     * A correction after a finished shootout
     * reopens the shootout.
     */
    if (
      delta < 0 &&
      m.status === 'finished'
