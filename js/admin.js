...
function renderAdminCard(m) {
  ...
  let pensBlock = '';
  if (pensOn) {
    const phase = (ph + pa) >= 10 && ph === pa ? 'Sudden death' : 'Pens (5 each, then SD if level)';
    pensBlock = `
        <div class="mt-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30">
          <div class="text-xs text-amber-400 font-bold mb-1">PENALTIES · ${phase}</div>
          <div class="text-2xl font-extrabold text-amber-300">${ph} – ${pa}</div>
        </div>`;
  } else if (hs === as) {
...
window.startPens = async function(matchId) {
  ...
  if (error) alert('Error (run SQL_PENS.md?): ' + error.message);
  else loadAdminData();
};
